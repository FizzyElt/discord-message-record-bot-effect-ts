...
const isDiscordInviteLink = (msgString: string) => discordInviteRegex.test(msgString);
...
export const inviteLinkGuard = (
    ...
    | Effect.Effect<OmitPartialGroupDMChannel<Message<boolean>>, string, never> => {
    if (String.isString(msg.content) && isDiscordInviteLink(msg.content) && msg.deletable) {
